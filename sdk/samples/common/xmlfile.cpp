...
CXmlFile::CXmlFile() : xml_doc_(NULL), create_new_file_(false) {
}
...
bool CXmlFile::ReadHeader() {
  ...
  if (elem->Value() == kSkpToXMLTag) {
    int version = 0;
    ok = (elem->QueryIntAttribute(kXMLVersionTag.c_str(), &version) == tinyxml2::XML_NO_ERROR) &&
         (version == 3);
  }
  return ok;
}
...
bool CXmlFile::ReadComponentDefinitionInfo(
    const tinyxml2::XMLNode* parent_node, bool readEntities,
...
bool CXmlFile::ReadLayerInfo(const tinyxml2::XMLNode* parent_node, XmlLayerInfo& info) const {
...
bool CXmlFile::ReadColor(const tinyxml2::XMLNode* parent_node, const SUColor& color) const {
...
bool CXmlFile::ReadMaterialInfo(const tinyxml2::XMLNode* parent_node, XmlMaterialInfo& info) const {
  const tinyxml2::XMLElement* elem = parent_node->ToElement();
  ...
  // Alpha (optional)
  info.has_alpha_ =
      elem->QueryDoubleAttribute(kAlphaTag.c_str(), &info.alpha_) == tinyxml2::XML_NO_ERROR;
  ...
  if (child != NULL) {
    ...
    if (child_elem->Value() == kTextureTag) {
      ...
      ok &= child_elem->QueryDoubleAttribute(kSScaleTag.c_str(), &info.texture_sscale_) ==
            tinyxml2::XML_NO_ERROR;
      ok &= child_elem->QueryDoubleAttribute(kTScaleTag.c_str(), &info.texture_tscale_) ==
            tinyxml2::XML_NO_ERROR;
    }
  }
...
static bool ReadPoint(const tinyxml2::XMLNode* parent_node, CPoint3d& point) {
  const tinyxml2::XMLElement* elem = parent_node->ToElement();
  double x, y, z;
  if (elem->QueryDoubleAttribute(kXTag.c_str(), &x) == tinyxml2::XML_NO_ERROR &&
      elem->QueryDoubleAttribute(kYTag.c_str(), &y) == tinyxml2::XML_NO_ERROR &&
      elem->QueryDoubleAttribute(kZTag.c_str(), &z) == tinyxml2::XML_NO_ERROR) {
...
bool CXmlFile::ReadEdgeInfo(const tinyxml2::XMLNode* parent_node, XmlEdgeInfo& info) const {
  // Layer (optional)
  const tinyxml2::XMLNode* child = parent_node->FirstChild();
  if (child == NULL)
    return false;
...
bool CXmlFile::ReadFaceInfo(const tinyxml2::XMLNode* parent_node, XmlFaceInfo& info) const {
  ...
  if (child->Value() == kLoopTag) {
    ...
    const tinyxml2::XMLElement* elem = child->ToElement();
    ok = elem->QueryIntAttribute(kCountTag.c_str(), &triangle_count) == tinyxml2::XML_NO_ERROR;
  }
  if (ok) {
        ...
        if (ReadPoint(pt_node, vertex.vertex_)) {
          ...
          if (info.has_front_texture_) {
            ...
            if (node != NULL && node->Value() == kFrontTextureCoordsTag) {
              elem = node->ToElement();
              double u, v;
              if (elem->QueryDoubleAttribute(kUTag.c_str(), &u) == tinyxml2::XML_NO_ERROR &&
                  elem->QueryDoubleAttribute(kVTag.c_str(), &v) == tinyxml2::XML_NO_ERROR) {
...
void CXmlFile::WriteFaceInfo(const XmlFaceInfo& info) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (info.has_front_texture_) {
      tinyxml2::XMLElement* elem = WriteStartTag(kFrontTextureCoordsTag.c_str());
...
bool CXmlFile::ReadCurveInfo(const tinyxml2::XMLNode* parent_node, XmlCurveInfo& info) const {
  bool ok = true;
  const tinyxml2::XMLNode* child = parent_node->FirstChild();
  while (child != NULL) {
...
void CXmlFile::WriteCurveInfo(const XmlCurveInfo& info) {
  ...
  for (std::vector<XmlEdgeInfo>::const_iterator it = info.edges_.begin(); it != info.edges_.end();
       ++it) {
    WriteEdgeInfo(*it);
  }
...
bool CXmlFile::ReadTransformation(
    const tinyxml2::XMLNode* parent_node, SUTransformation& transform) const {
...
bool CXmlFile::ReadLayers(
    const tinyxml2::XMLNode* parent_node, std::vector<XmlLayerInfo>& layer_infos) const {
  bool ok = true;
  const tinyxml2::XMLNode* child = parent_node->FirstChild();
  while (child != NULL) {
    XmlLayerInfo info;
...
bool CXmlFile::ReadMaterials(
    const tinyxml2::XMLNode* parent_node, std::vector<XmlMaterialInfo>& mat_infos) const {
  bool ok = true;
  const tinyxml2::XMLNode* child = parent_node->FirstChild();
  while (child != NULL) {
...
bool CXmlFile::ReadComponentDefinitions(
...
void CXmlFile::WriteComponentInstanceInfo(const XmlComponentInstanceInfo& info) {
...
bool CXmlFile::ReadComponentInstanceInfo(
    const tinyxml2::XMLNode* parent_node, XmlComponentInstanceInfo& info) const {
  bool ok = true;
...
bool CXmlFile::ReadEntities(const tinyxml2::XMLNode* parent_node, XmlEntitiesInfo& entities) const {
  bool ok = true;
